...
if ($browseDirectories) {
  ...
  $_GET['b'] = str_replace(array(
    '/..',
    '../'
  ), '', $_GET['b']); // Avoid going up into filesystem
...
function sortByName($a, $b) {
  return ($a['isdir'] == $b['isdir'] ? strtolower($a['name']) > strtolower($b['name']) : $a['isdir'] < $b['isdir']);
}

function sortBySize($a, $b) {
  return ($a['isdir'] == $b['isdir'] ? $a['size'] > $b['size'] : $a['isdir'] < $b['isdir']);
}
...
function getTitle($title) {
  ...
  $path = $_path;
  if (!empty($_browse)) {
    $path = $path . ($path != '/' ? '/' : '') . $_browse;
  }
  return str_replace(array('{{path}}', '{{files}}', '{{size}}'), array(
    $path,
    $_total,
    humanizeFilesize($_total_size, $sizeDecimals)
  ), $title);
}
...
<div id="wrapper">
    ...
    <ul id="header">
        ...
        <li>
            <a href="<?php echo buildLink(array(
              's' => 'size',
              'r' => (!$_sort_reverse && $_sort == 'size') ? '1' : NULL
            )) ?>"
               class="size <?php if ($_sort == 'size') echo $_sort_reverse ? 'desc' : 'asc' ?>"><span>Size</span></a>
            <a href="<?php echo buildLink(array(
              's' => 'time',
              'r' => (!$_sort_reverse && $_sort == 'time') ? '1' : NULL
            )) ?>"
               class="date <?php if ($_sort == 'time') echo $_sort_reverse ? 'desc' : 'asc' ?>"><span>Created (UTC +2)</span></a>
            <a href="<?php echo buildLink(array(
              's' => NULL,
              'r' => (!$_sort_reverse && $_sort == 'name') ? '1' : NULL
            )) ?>"
               class="name <?php if ($_sort == 'name') echo $_sort_reverse ? 'desc' : 'asc' ?>"><span>Name</span></a>
        </li>
      ...
      <?php foreach ($items as $item): ?>
          ...
          <li class="item">
            ...
            ?>
              ...
              <a href="<?php echo htmlentities($itemURL) ?>"
                 class="name <?php if ($showIcons) echo $item['isdir'] ? 'directory' : 'file' ?>"><?php echo htmlentities($item['name']) . ($item['isdir'] ? ' /' : '') ?></a>
